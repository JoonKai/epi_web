...
def login(form: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    if user.session_expire_minutes:
        expire_minutes = user.session_expire_minutes
    else:
...
def refresh(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """활동 중인 사용자의 토큰을 갱신해 세션을 연장한다."""
    if current_user.session_expire_minutes:
        expire_minutes = current_user.session_expire_minutes
    else:
        setting = db.query(SystemSetting).filter(SystemSetting.key == "session_expire_minutes").first()
        expire_minutes = int(setting.value) if setting else ACCESS_TOKEN_EXPIRE_MINUTES_DEFAULT
    token = create_access_token({"sub": current_user.username, "role": current_user.role}, expire_minutes=expire_minutes)
    return {"access_token": token, "expire_minutes": expire_minutes}
