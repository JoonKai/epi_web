...
function RiskBar({ name, remaining, daily_usage }) {
  ...
  return (
    <Tooltip title={daysLeft == null ? `${name}: 사용량 미입력` : `${name}: 약 ${Math.max(daysLeft, 0).toFixed(0)}일 후 임계`}>
      <div style={{ marginBottom: 8 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, fontSize: 11, color: 'var(--nowa-text-muted)', marginBottom: 4 }}>
          <span>{name}</span>
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 'auto', color: 'var(--nowa-text-muted)', fontSize: 12 }}>
        {forcedDown ? '강제 다운 해제 전까지 현황판에서 다운 상태로 유지됩니다.' : '현재 장비 상태를 기반으로 리스크를 표시합니다.'}
      </div>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ width: 30, height: 30, borderRadius: 9, background: `${resolvedAccent}18`, border: `1px solid ${resolvedAccent}30`, color: resolvedAccent, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, flexShrink: 0 }}>
          {icon}
        </div>
      </div>
...
function ForceDownTab({ machineList, pendingForcedDownMap, onToggle, onSave, saving }) {
...
function OverviewTab({ machineList, filtered, filter, setFilter, search, setSearch, fetchAll }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} md={8}><SummaryCard label="가동 설비" value={activeCount} suffix="대" accent="#818cf8" gradient="linear-gradient(135deg,#6366f1 0%,#8b5cf6 100%)" icon={<ToolOutlined />} sub="현재 운영 중인 설비" /></Col>
        <Col xs={24} md={8}><SummaryCard label="다운 설비" value={downCount} suffix="대" accent="#fb7185" gradient="linear-gradient(135deg,#f43f5e 0%,#ec4899 100%)" icon={<AlertOutlined />} sub="강제 다운 포함" /></Col>
        <Col xs={24} md={8}><SummaryCard label="주의 설비" value={warningCount} suffix="대" accent="#fbbf24" gradient="linear-gradient(135deg,#f59e0b 0%,#f97316 100%)" icon={<WarningOutlined />} sub="단기 점검 필요 대상" /></Col>
      </Row>

      <Row gutter={[16, 16]}>
        <Col xs={24} md={12}>
          <Card className="nowa-card" title={<span style={{ fontSize: 15, fontWeight: 800 }}>장비 다운 위험 현황</span>} styles={{ body: { padding: '8px 12px 4px' }, header: { minHeight: 52 } }}>
            {riskChartRows.length === 0 ? (
              ...
              <ReactECharts
                ...
                option={{
                  ...
                  xAxis: { type: 'category', data: riskChartRows.map((row) => `${row.machine_no}`), axisLabel: { color: '#64748b', fontSize: 10, rotate: 30 }, axisLine: { lineStyle: { color: '#1e2a3c' } } },
          ...
          <Card className="nowa-card" title={<span style={{ fontSize: 15, fontWeight: 800 }}>다운 예상 시기 분포</span>} styles={{ body: { padding: '8px 12px 4px' }, header: { minHeight: 52 } }}>
            <ReactECharts
              ...
              option={{
                ...
                tooltip: { trigger: 'axis' },
                xAxis: { type: 'category', data: distributionBuckets.map((bucket) => bucket.label), axisLabel: { color: '#64748b', fontSize: 11, rotate: 20 }, axisLine: { lineStyle: { color: '#1e2a3c' } } },
                yAxis: { type: 'value', minInterval: 1, axisLabel: { color: '#64748b', fontSize: 11 }, splitLine: { lineStyle: { color: '#1e2a3c' } } },
                series: [
                  {
                    type: 'bar',
      ...
      <Card className="nowa-card" styles={{ body: { padding: '14px 18px', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' } }}>
        <Input prefix={<SearchOutlined style={{ color: '#64748b' }} />} placeholder="호기 검색" value={search} onChange={(event) => setSearch(event.target.value)} allowClear style={{ width: 180 }} />
...
export default function MocvdManagement() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      setError('MOCVD 장비 데이터를 불러오지 못했습니다.')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const filtered = useMemo(() => {
    ...
    return machineList.filter((machine) => {
      if (keyword && !`${machine.machine_no}`.includes(keyword) && !formatMachineLabel(machine.machine_no).toLowerCase().includes(keyword) && !(machine.description ?? '').toLowerCase().includes(keyword)) {
        return false
      }
